...
const  int        MAX_RETRY           = 5;      // 최대 자동 재시도 횟수
...
static void send_range() {
    ...
    Serial.printf("[UWB] RANGE TX 완료(DELAYED) → RANGE_REPORT 대기  at %lu ms\n", millis());
...
static void process_uwb() {
    ...
    if (rangeState == RangeState::PENDING_RANGE_SEND &&
        ...
        deadlineMs = millis() + RANGE_TIMEOUT_MS;
        send_range();
        Serial.printf("[UWB] POLL_ACK 수신 후 %dms 대기 → RANGE 전송  (rxCount=%lu)\n",
